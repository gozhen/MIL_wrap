function [X_inst, y_inst, y_bag] = ...
    f_genData_MilCount_regress_nonPar( seqs,labels, k,s,cs, is01)
...
% k: instance length
% s: instance shift range
% cs: kmer length (feature)
...
textprogressbar('Generating feature vectors:             ');
ratio = round(nBags / 100);
% ------------------------------- par for ---------------------------------
%parfor i=1:nBags
for i=1:nBags
    ...
    %if rem(i, 1000) == 0 
        %fprintf('%d \n', i);
    %end
    %fprintf('  generating feature vector for %d of %d sequences.\r', i);
